...
std::pair<std::string, DnaSequence* > DataHandler::find_by_name(std::string name) {
...
std::pair<std::string, DnaSequence* > DataHandler::find_by_id(std::string id) {
    /*remove # from the id*/
    id.erase(0, 1);
...
std::pair<std::string, DnaSequence* > DataHandler::fetch_data_and_return(std::string id, std::string name) {

    std::stringstream s;
    s << id << " " << name;
    std::pair<std::string, DnaSequence* > m(std::make_pair(s.str(), s_dataHolder[id])) ;
